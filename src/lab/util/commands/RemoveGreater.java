...
/**
 * This command allows to remove all locations from collection on server which has more items than specified location.
 ...
 * @since 1.7.0
 */
public class RemoveGreater extends DBCommand {

    public RemoveGreater(String login, String password, byte[] argument) {
        super(login, password, argument);
    }

    public RemoveGreater(byte[] login, byte[] password, byte[] argument) {
        super(login, password, argument);
    }
    ...
    @Override
    public int getCode() {
        return Commands.getCommandCode("remove_greater");
    }
